...
import math.Integrators.Parabola_Integrator;
import math.math_primitives.Radius;

public class Polar_Arclenght_Calculator extends Arclenght_Calculator{


    public Polar_Arclenght_Calculator(Radius radius) {
        super(radius);
    }
    ...
    public double calculate_arclenght(double left_bound, double right_bound,int num_of_separations) {
        ...
        Radius elementary_arclenght = new Radius() {
            ...
            public double get_radius(double x) {
                double f = radius.get_radius(x);
                double f_prime = radius.get_derivative().get_radius(x);
                return Math.sqrt(f*f + f_prime*f_prime);
            }
